...
runmean(X::AbstractMatrix; n::Int = 10, cumulative::Bool = true)::Matrix{Float64} =
    hcat((runmean(X[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...)
...
runsum(X::AbstractMatrix; n::Int = 10, cumulative::Bool = true)::Matrix{Float64} =
    hcat((runsum(X[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...)
...
runmad(X::AbstractMatrix; n::Int = 10, cumulative::Bool = true)::Matrix{Float64} =
    hcat((runmad(X[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...)
...
runvar(X::AbstractMatrix; n::Int = 10, cumulative::Bool = true)::Matrix{Float64} =
    hcat((runvar(X[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...)
...
runsd(X::AbstractMatrix; n::Int = 10, cumulative::Bool = true)::Matrix{Float64} =
    sqrt.(hcat((runvar(X[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...))
...
runcov(
    ...
    (runcov(X[:, j], Y[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...,
)
...
runcor(
    ...
    (runcor(X[:, j], Y[:, j], n = n, cumulative = cumulative) for j in 1:size(X, 2))...,
)
runcor(
    X::AbstractMatrix,
    y::AbstractVector;
    ...
    hcat((runcor(X[:, j], y, n = n, cumulative = cumulative) for j in 1:size(X, 2))...)
...
function runmax(
    ...
    if inclusive
        ...
        if cumulative
            ...
            @inbounds for i in (n+1):(size(x, 1)-1)
                out[i+1] = max(out[i-1], x[i-1])
            end
        else
            @inbounds for i in n:(size(x, 1)-1)
...
runmax(
    ...
    (
        runmax(X[:, j], n = n, cumulative = cumulative, inclusive = inclusive) for
        j in 1:size(X, 2)
    )...,
)
...
runmin(
    ...
    (
        runmin(X[:, j], n = n, cumulative = cumulative, inclusive = inclusive) for
        j in 1:size(X, 2)
    )...,
)
...
runquantile(
    ...
    (runquantile(X[:, j], n = n, cumulative = cumulative, p = p) for j in 1:size(X, 2))...,
)
...
runacf(
    ...
    (
        runacf(X[:, j], n = n, cumulative = cumulative, maxlag = maxlag, lags = lags)
        for j in 1:size(X, 2)
    )...,
)
