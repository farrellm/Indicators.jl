...
function hlc_fun(X::TS, f::Function, flds::Vector{Symbol}; args...)
    ...
    elseif size(X, 2) > 3 && has_high(X) && has_low(X) && has_close(X)
        ...
        error(
            "Argument must have 3 columns or have High, Low, and Close/Settle/Last fields.",
        )
    end
end
...
function ohlc_fun(X::TS, f::Function, flds::Vector{Symbol}; args...)
    ...
    elseif size(X, 2) > 4 && has_open(X) && has_high(X) && has_low(X) && has_close(X)
        ...
        error(
            "Argument must have 4 columns or have Open, High, Low, and Close/Settle/Last fields.",
        )
    end
end
...
runacf(
    ...
    X,
    runacf,
    [Symbol(i) for i in lags];
    n = n,
    maxlag = maxlag,
    lags = lags,
    cumulative = cumulative,
)
runfun(X::TS, f::Function; n::Int = 10, cumulative::Bool = true, args...) =
    TS(runfun(X, f, n = n, cumulative = cumulative, args...), X.index, [:Function])
...
ichimoku(X::TS; args...) =
    hlc_fun(X, ichimoku, [:Tenkan, :Kijun, :SenkouA, :SenkouB, :Chikou]; args...)
...
keltner(X::TS; args...) =
    hlc_fun(X, keltner, [:KeltnerLower, :KeltnerMiddle, :KeltnerUpper]; args...)
